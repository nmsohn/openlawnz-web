...
const ModuleSelector = ({ module, idx, wizardModuleData }) => {
  ...
  const errorModule = (message) => {
    return (
      <div className="microsite-paragraph" name={title}>
        <h2>{module.title}</h2>
        <p>Error: {message}</p>
      </div>
    )
      ...
      return (
        <div name={title} className="module-block">
          ...
          {module.content.map(({ content_html }, idx) => {
            return <p key={idx} className="microsite-paragraph" dangerouslySetInnerHTML={{ __html: content_html }} />
          })}
        </div>
      )
    case "faqs":
